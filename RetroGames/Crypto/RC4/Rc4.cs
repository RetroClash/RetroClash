...
namespace RetroGames.Crypto.RC4
{
    ...
    public class Rc4Core
    {
        ...
        public void UpdateCiphers(string key, uint clientSeed, byte[] serverNonce)
        {
            if (serverNonce == null)
                throw new ArgumentNullException(nameof(serverNonce));

            InitializeCiphers(Encoding.UTF8.GetBytes(key + ScrambleNonce(clientSeed, serverNonce)));
        }
